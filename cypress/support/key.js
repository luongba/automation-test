Cypress.Commands.add("loginAndSearchProject", () => {
  cy.fixture("account.json").then((data) => {
    cy.visit("https://10.240.234.71");
...
Cypress.Commands.add("baseCreateKey", (fileData) => {
  cy.loginAndSearchProject();
  cy.visit("https://10.240.234.71/key-manager/keys/AddKeys");
...
Cypress.Commands.add("keySearch", (fileData) => {
  cy.loginAndSearchProject(fileData);
  cy.visit("https://10.240.234.71/key-manager/keys");
  cy.fixture(fileData).then((data) => {
...
Cypress.Commands.add("createKey", (fileData) => {
  ...
  return cy.url().then((currentURL) => {
    if (currentURL === "https://10.240.234.71/key-manager/keys") {
      cy.fixture(fileData).then((data) => {
